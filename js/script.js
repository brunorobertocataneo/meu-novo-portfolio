document.addEventListener('DOMContentLoaded', () => {
	...
	const markDownloadUnavailable = link => {
		link.classList.add('is-unavailable');
		link.setAttribute('aria-disabled', 'true');
		link.setAttribute(
			'title',
			'PDF não encontrado. Adicione o arquivo na pasta assets/downloads indicada no projeto.'
		);
		link.addEventListener('click', event => {
			event.preventDefault();
			window.alert(
				'O arquivo ainda não está no projeto.\n\nColoque o PDF em:\n' + link.getAttribute('href')
			);
		});
	};

	const checkDownloadFile = link => {
		const href = link.getAttribute('href');
		if (!href || window.location.protocol === 'file:') return;

		fetch(href, { method: 'HEAD' })
			.then(response => {
				if (!response.ok) markDownloadUnavailable(link);
			})
			.catch(() => markDownloadUnavailable(link));
	};

	document.querySelectorAll('[data-cert-download], [data-download-curriculo]').forEach(checkDownloadFile);
});
